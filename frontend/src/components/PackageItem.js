...
const editPackage = () => {
    ...
    const onSubmit = (e, id) => {
        e.preventDefault();
        dispatch(updatePackage({_id: id, name: nameEdit, description: descriptionEdit, link: linkEdit, category: categoryEdit, userId: npmpackage.userId }));
    }
...
<form method="POST" id="package_form" onSubmit={(e) => onSubmit(e, _id)}></form>
    ...
    <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
        <thead className="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
            <tr>
                <th scope="col" className="px-6 py-3 border-r-2	border-white">
                    Name
                </th>
                <th scope="col" className="px-6 py-3 border-r-2	border-white">
                    Description
        ...
        <tbody>

            {
                !isEdit? (
                    ...
                    <td className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                        <input className="border-2 border-gray-300 rounded p-1" type="text" value={categoryEdit} form="package_form" onChange={(e) => setCategory(e.target.value)} />
                    </td>
                    <td className="px-6 py-4 text-right">
                        <button onClick={() => editPackage()} className="font-medium text-blue-600 dark:text-blue-500 hover:underline">Edit</button>
                    </td>
